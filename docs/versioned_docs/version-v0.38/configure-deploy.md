...
1. An access rule allowing anonymous access to
   `https://httpbin.org/anything/header` using the `cookie` mutator.
...
```sh
$ docker run oryd/oathkeeper:v0.38.3-beta.1 credentials generate --alg RS256 > jwks.json
```
...
```shell
$ cat << EOF > Dockerfile
FROM oryd/oathkeeper:v0.38.3-beta.1
