...
```sh
$ docker run oryd/oathkeeper:v0.38.5-beta.1.pre.1 credentials generate --alg RS256 > jwks.json
```
...
```shell
$ cat << EOF > Dockerfile
FROM oryd/oathkeeper:v0.38.5-beta.1.pre.1
...
```sh
$ docker pull oryd/oathkeeper:v0.38.5-beta.1.pre.1
```
