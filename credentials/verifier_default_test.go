...
func TestVerifierDefault(t *testing.T) {
	...
	var sign = func(claims jwt.MapClaims, src string) string {
		tt, err := signer.Sign(context.Background(), x.ParseOrPanic(src), claims)
	...
	for k, tc := range []struct {
		...
		},
		{
			...
			c: &ValidationContext{
				...
				KeyURLs:       []url.URL{*x.ParseOrPanic("file://../test/stub/jwks-hs.json")},
		...
		},
		{
			...
			c: &ValidationContext{
				...
				Issuers:       []string{"iss-1", "iss-2"},
				Scope:         []string{"scope-1", "scope-2"},
				KeyURLs:       []url.URL{*x.ParseOrPanic("file://../test/stub/jwks-rsa-single.json")},
				ScopeStrategy: fosite.ExactScopeStrategy,
			},
			token: sign(jwt.MapClaims{
				"sub":   "sub",
