...
func TestFetcherDefault(t *testing.T) {
	...
	s := credentials.NewFetcherDefault(reg, maxWait, JWKsTTL)
	...
	t.Run("name=should result in error because server times out", func(t *testing.T) {
		_, err := s.ResolveKey(t.Context(), uris, "c61308cc-faef-4b98-99c3-839f513ac296", "sig")
		require.Error(t, err)
	})

	t.Run("name=should result in error because key id does not exist", func(t *testing.T) {
		_, err := s.ResolveKey(t.Context(), uris, "i-do-not-exist", "sig")
		require.Error(t, err)
	})

	t.Run("name=should find the key even if the server is slow", func(t *testing.T) {
		key, err := s.ResolveKey(t.Context(), uris, "2aeaef79-7233-4a59-95bf-e32151d3544b", "sig")
	...
	t.Run("name=should find the key when the server works normally and when it sends multiple keys", func(t *testing.T) {
		key, err := s.ResolveKey(t.Context(), uris, "392e1a6b-6ae1-48b8-bea3-2fe09447805c", "sig")
		require.NoError(t, err)
		assert.Equal(t, "392e1a6b-6ae1-48b8-bea3-2fe09447805c", key.KeyID)

		key, err = s.ResolveKey(t.Context(), uris, "8e884167-1300-4f58-8cc1-81af68f878a8", "sig")
		require.NoError(t, err)
	...
	t.Run("name=should find the previously timed out key because enough time has passed", func(t *testing.T) {
		key, err := s.ResolveKey(t.Context(), uris, "c61308cc-faef-4b98-99c3-839f513ac296", "sig")
		require.NoError(t, err)
	...
	t.Run("name=should fetch keys from the file system", func(t *testing.T) {
		t.Parallel()
		key, err := s.ResolveKey(t.Context(), uris, "81be3441-5303-4c52-b00d-bbdfadc75633", "sig")
		require.NoError(t, err)
		assert.Equal(t, "81be3441-5303-4c52-b00d-bbdfadc75633", key.KeyID)

		key, err = s.ResolveKey(t.Context(), uris, "3e0edde4-12ad-425d-a783-135f46eac57e", "sig")
		require.NoError(t, err)
		assert.Equal(t, "3e0edde4-12ad-425d-a783-135f46eac57e", key.KeyID)

		key, err = s.ResolveKey(t.Context(), uris, "f4190122-ae96-4c29-8b79-56024e459d80", "sig")
		require.NoError(t, err)
	...
	t.Run("name=should resolve all the json web key sets", func(t *testing.T) {
		sets, err := s.ResolveSets(t.Context(), uris)
		require.NoError(t, err)
	...
	t.Run("name=should fetch from s3 object storage", func(t *testing.T) {
		t.Parallel()
		s := credentials.NewFetcherDefault(reg, maxWait, JWKsTTL, credentials.WithURLMux(cloudstorage.NewTestURLMux(t)))

		key, err := s.ResolveKey(t.Context(), []url.URL{
	...
	t.Run("name=should fetch from azure object storage", func(t *testing.T) {
		...
		jwkKey, err := s.ResolveKey(t.Context(), []url.URL{
