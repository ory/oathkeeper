...
func TestAuthenticatorCookieSession(t *testing.T) {
	...
	t.Run("method=authenticate-with-cache", func(t *testing.T) {
		...
		assertHandlerWasCalled := func(t *testing.T) {
			assert.Equal(t, 1, handlerCallCount, "expected the handler to have been called once")
			handlerCallCount = 0
		}
		assertCacheWasUsed := func(t *testing.T) {
			assert.Equal(t, 0, handlerCallCount, "expected the cache to have been used")
			handlerCallCount = 0
		}

		t.Run("case=cache is disabled by default", func(t *testing.T) {
			testServer, _ := makeServer(t, 200, `{"subject": "123", "extra": {"foo": "bar"}}`)
			req := makeRequest("GET", "/", "", map[string]string{"sessionid": "abc123"}, "")
		...
		t.Run("case=initial request succeeds and caches", func(t *testing.T) {
			testServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				handlerCallCount++
				w.WriteHeader(http.StatusOK)
				w.Write([]byte(`{"subject": "cached-user", "extra": {"role": "admin"}}`))
			}))
			t.Cleanup(testServer.Close)

			req := makeRequest("GET", "/", "", map[string]string{"sessionid": "cache-test-123"}, "")

			err := pipelineAuthenticator.Authenticate(
				req,
				session,
				json.RawMessage(fmt.Sprintf(`{"check_session_url": "%s", "cache": {"enabled": true, "ttl": "60s"}}`, testServer.URL)),
				nil,
			)
			require.NoError(t, err)
			assert.Equal(t, "cached-user", session.Subject)
		...
		t.Run("case=second request uses cache", func(t *testing.T) {
			...
			req := makeRequest("GET", "/", "", map[string]string{"sessionid": "cache-test-456"}, "")
			...
			sess2 := new(AuthenticationSession)
			err = pipelineAuthenticator.Authenticate(
				req,
				sess2,
				json.RawMessage(fmt.Sprintf(`{"check_session_url": "%s", "cache": {"enabled": true, "ttl": "60s"}}`, testServer.URL)),
			...
			require.NoError(t, err)
			assert.Equal(t, "cached-user", sess2.Subject)
			assert.Equal(t, map[string]interface{}{"role": "admin"}, sess2.Extra)
			assertCacheWasUsed(t)
		})

		t.Run("case=different cookies result in different cache keys", func(t *testing.T) {
			testServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				handlerCallCount++
				cookie, _ := r.Cookie("sessionid")
				w.WriteHeader(http.StatusOK)
				w.Write([]byte(fmt.Sprintf(`{"subject": "user-%s", "extra": {}}`, cookie.Value)))
			}))
			t.Cleanup(testServer.Close)

			req1 := makeRequest("GET", "/", "", map[string]string{"sessionid": "user1"}, "")
			sess1 := new(AuthenticationSession)
			err := pipelineAuthenticator.Authenticate(
				req1,
				sess1,
				json.RawMessage(fmt.Sprintf(`{"check_session_url": "%s", "cache": {"enabled": true, "ttl": "60s"}}`, testServer.URL)),
			...
			require.NoError(t, err)
			assert.Equal(t, "user-user1", sess1.Subject)
			assertHandlerWasCalled(t)

			req2 := makeRequest("GET", "/", "", map[string]string{"sessionid": "user2"}, "")
			sess2 := new(AuthenticationSession)
			err = pipelineAuthenticator.Authenticate(
				req2,
				sess2,
			...
			require.NoError(t, err)
			assert.Equal(t, "user-user2", sess2.Subject)
			assertHandlerWasCalled(t)
		})
		...
		t.Run("case=cache expires after TTL", func(t *testing.T) {
			testServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				handlerCallCount++
				w.WriteHeader(http.StatusOK)
				w.Write([]byte(`{"subject": "ttl-test", "extra": {}}`))
			}))
			t.Cleanup(testServer.Close)

			req := makeRequest("GET", "/", "", map[string]string{"sessionid": "ttl-test"}, "")
			sess := new(AuthenticationSession)

			err := pipelineAuthenticator.Authenticate(
				req,
				sess,
				json.RawMessage(fmt.Sprintf(`{"check_session_url": "%s", "cache": {"enabled": true, "ttl": "100ms"}}`, testServer.URL)),
				nil,
			)
			require.NoError(t, err)
			assertHandlerWasCalled(t)
			...
			sess3 := new(AuthenticationSession)
			err = pipelineAuthenticator.Authenticate(
				req,
				sess3,
				json.RawMessage(fmt.Sprintf(`{"check_session_url": "%s", "cache": {"enabled": true, "ttl": "100ms"}}`, testServer.URL)),
