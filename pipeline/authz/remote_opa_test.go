...
func TestAuthorizerRemoteOPAAuthorize(t *testing.T) {
	tests := []struct {
		name    string
		...
		},
		{
			name:    "invalid json",
			session: &authn.AuthenticationSession{},
			config:  json.RawMessage(`{"remote":"http://host/path","payload":"{{ .PathArray }}]"}`),
			wantErr: true,
		},
		{
			name:    "unknown field",
		...
		},
		{
			...
			config: json.RawMessage(`{"payload":"{\"subject\":\"{{ .Subject }}\",\"extra\":\"{{ .Extra.foo }}\",\"method\":\"{{ .MatchContext.Method }}\",\"match\":\"{{ index .MatchContext.RegexpCaptureGroups 0 }}\"}"}`),
		},
		{
			name: "json array",
			setup: func(t *testing.T) *httptest.Server {
...
func TestAuthorizerRemoteOPAValidate(t *testing.T) {
	tests := []struct {
		name    string
		enabled bool
		config  json.RawMessage
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			p := configuration.NewViperProvider(logrusx.New("", ""))
			a := NewAuthorizerRemoteOPA(p)
			viper.Set(configuration.ViperKeyAuthorizerRemoteOPAIsEnabled, tt.enabled)
